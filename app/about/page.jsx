const AboutPage = () => {
  return (
    <section className="wrapper space-y-5">
      <h2 className="text-2xl font-medium text-amber-500">About</h2>
      <p>
        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sequi, modi
        voluptatibus laudantium accusantium libero accusamus sapiente
        perspiciatis ullam ipsum impedit qui temporibus dolor nisi deserunt
        dolorum, veniam ducimus vero voluptate molestiae. Voluptates similique
        omnis amet, architecto tenetur sit, nobis itaque expedita sed ipsam,
        quis explicabo aliquid. Ullam ipsum adipisci doloribus?
      </p>
      <p>
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Provident
        tempore quae aut sequi error, inventore velit debitis quam et
        laboriosam. Ipsa laborum eos libero tempora aliquam nostrum expedita ea
        sequi iusto asperiores voluptatibus, quasi facilis.
      </p>
      <p>
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Nesciunt nemo
        beatae esse aspernatur minima. Id voluptatem dolores, ea eos asperiores
        quia velit aliquid quas laborum, debitis magnam voluptate, voluptates
        aperiam ratione facere! Veniam id dolorem velit? Commodi quas aut,
        repudiandae at voluptatem ut quibusdam dolore.
      </p>
    </section>
  );
};

export default AboutPage;
